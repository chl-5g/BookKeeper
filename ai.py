"""AI 功能模块 — 通用 LLM API 接口（OpenAI 兼容 / Anthropic 自动检测）"""
...
# 自动检测 API 类型
_IS_ANTHROPIC = "anthropic" in API_BASE_URL.lower()


def _api_url(stream: bool = False) -> str:
    """返回完整的 API endpoint URL"""
    base = API_BASE_URL.rstrip("/")
    if _IS_ANTHROPIC:
        return base + "/v1/messages"
    # OpenAI-compatible: 支持已含 /v1 或仅域名
    if base.endswith("/v1"):
...
ALL_CATEGORIES = "餐饮、交通、购物、娱乐、居住、医疗、教育、通讯、投资、人情、美容理发、工资、理财、红包、报销、兼职、奖金、退款、租金收入、生意、补贴、其他"
ALL_CATS_LIST = ALL_CATEGORIES.split("、")


def _build_request(prompt: str, temperature: float, max_tokens: int,
                   model: str | None, stream: bool) -> tuple[str, dict, dict]:
    """构建 API 请求，返回 (url, body, headers)。
    自动适配 Anthropic Messages API 和 OpenAI Chat Completions API。
    """
...
def _extract_text(data: dict) -> str:
    """从 API 响应提取文本（兼容 Anthropic 和 OpenAI 格式）"""
    if _IS_ANTHROPIC:
        for block in data.get("content", []):
            if block.get("type") == "text":
                return block["text"]
        return data["content"][0].get("text", str(data["content"]))
    else:
        # OpenAI-compatible
...
def _extract_stream_delta(event: dict) -> str | None:
    """从 SSE 事件提取 delta 文本，无内容返回 None"""
    if _IS_ANTHROPIC:
...
async def _call_llm(prompt: str, temperature=0.7, max_tokens=2000,
    ...
    """调用 LLM API，返回文本"""
    url, body, headers = _build_request(prompt, temperature, max_tokens, model, stream=False)
...
async def _stream_llm(prompt: str, temperature=0.7, max_tokens=2000,
                      model: str | None = None):
    """流式调用 LLM API，yield text delta"""
    url, body, headers = _build_request(prompt, temperature, max_tokens, model, stream=True)
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
...
_KEYWORD_MAP = {
    ...
    "购物": [
        ...
        "包包", "化妆品", "日用", "家电", "数码", "手机", "电脑",
        "充电", "耳机", "家具", "装修", "洗衣", "快递", "1688", "唯品会",
        "得物", "闲鱼", "苹果", "小米", "超市", "便利店", "便利",
    ],
    "娱乐": [
    ...
    "美容理发": [
        "理发", "剪头", "剪发", "美发", "染发", "烫发", "护肤", "美甲", "美容",
        "化妆", "面膜", "spa", "按摩", "头疗", "修面", "修眉",
        "修脚", "足疗", "采耳", "刮痧", "艾灸", "推拿",
    ],
    "工资": [
        "工资", "薪水", "月薪", "发工资", "薪资",
...
async def generate_report(month, income_total, expense_total,
                          income_cats, expense_cats, record_count,
                          data_version: int = 0) -> str:
...
语言简洁友好，不用表格。"""
    ...
    # 缓存：同月同数据不重复生成（data_version 确保数据变更后失效）
    cache_key = f"report:{month}:{income_total}:{expense_total}:{record_count}:{data_version}"
...
async def stream_report(month, income_total, expense_total,
                        income_cats, expense_cats, record_count,
                        data_version: int = 0):
    """流式生成月度报告，yield 每个 token"""
    cache_key = f"report:{month}:{income_total}:{expense_total}:{record_count}:{data_version}"
    cached = _cache_get(cache_key)
    if cached:
        yield cached
...
语言简洁友好，不用表格。"""
    ...
    full = []
    async for token in _stream_llm(prompt, max_tokens=800):
        full.append(token)
        yield token
    _cache_set(cache_key, "".join(full))
    ...
    if records:
        ...
        if amounts:
            ...
            if len(amounts) >= 3:
                ...
                threshold = max(avg + 2 * std, 500)
            else:
                threshold = max(avg * 5, 500)  # 数据太少时回退固定倍数
            seen_large = set()
            for r in records:
    ...
    if history_cats:
        ...
        for c in current_cats:
            ...
            if cat in hist_map and len(hist_map[cat]) >= 2:
                ...
                hist_std = math.sqrt(sum((a - hist_avg) ** 2 for a in hist_map[cat]) / len(hist_map[cat]))
                surge_threshold = hist_avg + 2 * hist_std
                if hist_avg > 0 and amt > surge_threshold and amt - hist_avg > 200:
                    alerts.append({
                        "type": "surge",
...
_QUERY_CAT_MAP = {
    ...
    "美容": "美容理发", "理发": "美容理发", "美甲": "美容理发", "护肤": "美容理发", "按摩": "美容理发",
}
...
async def spending_profile(months_data: list[dict],
                           data_version: int = 0) -> str:
    """分析多月数据，生成消费画像"""
    summary = ""
...
语言简洁有趣，像朋友聊天一样。"""
    ...
    # 缓存（data_version 确保数据变更后失效）
    cache_key = f"profile:{data_version}:" + hashlib.md5(summary.encode()).hexdigest()
    cached = _cache_get(cache_key)
    if cached:
        return cached
...
async def stream_profile(months_data: list[dict],
                          data_version: int = 0):
    """流式生成消费画像，yield 每个 token"""
    summary = ""
    for m in months_data:
        cats_str = "、".join(f"{c['category']}{c['amount']}元" for c in m.get("expense_cats", [])[:5])
        summary += f"{m['month']}：收入{m['income']:.0f}元，支出{m['expense']:.0f}元，主要支出：{cats_str}\n"
