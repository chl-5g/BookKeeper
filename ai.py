...
_NO_THINK = "/no_think"  # system prompt 减少 qwen3 思考 token
...
async def _call_llm(prompt: str, temperature=0.7, max_tokens=2000,
    ...
    # 给 thinking 预留 2000 token
    total_predict = max_tokens + 2000
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        resp = await client.post(OLLAMA_URL, json={
            "model": use_model,
            "messages": [
                {"role": "system", "content": _NO_THINK},
        ...
        # qwen3 把 thinking 和 content 分开，直接取 content
...
async def _stream_llm(prompt: str, temperature=0.7, max_tokens=2000,
                      model: str | None = None):
    """流式调用 ollama，只 yield content 字段（跳过 thinking）"""
    use_model = model or MODEL
    total_predict = max_tokens + 2000
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
